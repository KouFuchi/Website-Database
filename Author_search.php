...
<body>
    ...
    <br><br><br><br>
    <center><h3>查詢Author資料表</h3></center><br>
    <form action="" method="post" align="center" >
      ...
      <select name="option">
        ...
        <option value="2">作者/編者</option>
        <option value="3">領域</option>
        <option value="4">所屬機構</option>
      ...
      <input type="submit" value="搜尋">
       ...
       <?php
        ...
        if($option==1){       //根據前面選擇的選項(值)判斷要在哪個資料表中查詢
           ...
           $sql = "select * from `Author` where `Authorname` like '%$key%' or `Field` like '%$key%' or `Organization` like '%$key%'  order by Authorname ";
        }
         else if($option==2){$sql = "select * from `Author` where `Authorname` like '%$key%' order by Authorname ";
         }
         else if($option==3){$sql = "select * from `Author` where `Field` like '%$key%' order by Authorname ";
         }
         else if($option==4){$sql = "select * from `Author` where `Organization` like '%$key%' order by Authorname ";
         }
        ...
        if (isset($option) && isset($key)){     //以表格形式列出結果，條件為關鍵字及按鈕都有觸發
          ...
          if ($total==0){
             ...
             echo "<table  align='center' >" ;
              ...
              echo "<td style='vertical-align:middle;'><b>領域</b></td>";
              echo "<td style='vertical-align:middle;'><b>所屬機構</b></td>";
