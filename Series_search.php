...
<body>
    ...
    <br><br><br><br>
    <center><h3>查詢Series資料表</h3></center><br>
      ...
      <input type="submit" value="搜尋">
       ...
       <?php
        ...
        if($option==1){       //根據前面選擇的選項(值)判斷要在哪個資料表中查詢
           ...
           $sql = "select * from `Series` where `Seriesname` like '%$key%' order by Seriesname ";
        }
         else if($option==2){$sql = "select * from `Series` where `Seriesname` like '%$key%' order by Seriesname ";
         }
