    ...
    <?php
      ...
      if (empty($_GET)){
       show($conn);
        ...
        if (count($_GET) == 1){  //因為原本的主葉面只有一個修改按鈕
          ...
          echo '<input type="text" name="AuthorID" value="' . $AuthorID . '" />';  
          echo '<input type="text" name="Authorname" value="' . $Authorname . '" />';    
          echo '<input type="text" name="Field" value="' . $Field . '" />';
          echo '<input type="text" name="Organization" value="' . $Organization . '" />';
          ...
          $sql = "UPDATE Author SET Authorname ='$Authorname', Field ='$Field' , Organization = '$Organization' Where AuthorID = $AuthorID";
      ...
      function show($conn){
        ...
        while ($row != NULL){
           ...
           echo "<td><a href=Author_update.php?AuthorID=$AuthorID>修改</a></td>";
           echo "</tr>";
           $row = mysqli_fetch_row($result); 
        }
      }
