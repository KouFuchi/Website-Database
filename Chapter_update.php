    ...
    <?php
      ...
      if (empty($_GET)){
       show($conn);
        ...
        if (count($_GET) == 1){  //因為原本的主葉面只有一個修改按鈕
          ...
          $sql = "SELECT * FROM Chapters where ChapterID = $ChapterID";
          $result = mysqli_query($conn, $sql);
          $row = mysqli_fetch_row($result);
          list($ChapterID, $Chaptername, $Chapterauthor ) = $row;
     
          echo '<form name="form" action="" method="get">';
          echo '<input type="text" name="ChapterID" value="' . $ChapterID . '" />';  
          echo '<input type="text" name="Chaptername" value="' . $Chaptername . '" />';    
          echo '<input type="text" name="Chapterauthor" value="' . $Chapterauthor . '" />';
          ...
          $sql = "UPDATE Chapters SET Chaptername ='$Chaptername', Chapterauthor ='$Chapterauthor' Where ChapterID = $ChapterID";
      ...
      function show($conn){
        ...
        while ($row != NULL){
           ...
           echo "<td><a href=Chapter_update.php?ChapterID=$ChapterID>修改</a></td>";
           echo "</tr>";
           $row = mysqli_fetch_row($result); 
        }
      }
